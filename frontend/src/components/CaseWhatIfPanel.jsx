...
export default function CaseWhatIfPanel({ caseId }) {
  ...
  return <div className="panel whatif-panel"><h2 className="whatif-title">What-If — Hypothesis Sandbox</h2><p className="muted-copy">Evaluate a hypothesis against this case without changing verified evidence or its graph.</p><textarea className="hypothesis-textarea" rows={4} value={hypothesis} onChange={(e)=>setHypothesis(e.target.value)} placeholder="What if this timestamp is incorrect?"/><button className="whatif-run-btn" onClick={run} disabled={loading||!hypothesis.trim()}>{loading?"Evaluating…":"Run case-scoped analysis"}</button>{error&&<div className="upload-error">{error}</div>}{result&&<div className="whatif-result" aria-live="polite"><div className="case-analysis-result"><div className="case-analysis-heading"><span>Hypothesis assessment</span><span className={`ingested-mode-badge ${result.mode}`}>{result.mode} mode</span></div><FormattedText text={result.narrative}/></div></div>}</div>;
}
