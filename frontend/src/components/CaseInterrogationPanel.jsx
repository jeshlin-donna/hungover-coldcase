...
export default function CaseInterrogationPanel({ caseId }) {
  ...
  return <div className="panel interrogation-panel"><h2>Interrogation Co-Pilot</h2><p className="muted-copy">Generate evidence-grounded, non-accusatory interview questions from this case only.</p>
    ...
    {error && <div className="upload-error">{error}</div>}{result && <div className="strategy-box"><span className="strategy-label">Case-scoped strategy</span><FormattedText text={result.narrative} className="strategy-text"/><span className={`ingested-mode-badge ${result.mode}`}>{result.mode} mode</span></div>}
  </div>;
}
